...
class Llama3_2TemplateMeta(Llama3TemplateMeta):
    # 1. 起始符：通常保留 BOS (Begin Of Sentence) token
    # prefix: Prompt = field(default_factory=lambda: ['<|begin_of_text|>'])
    prefix: Prompt = field(default_factory=lambda: [''])
    # 2. 核心修改：定义用户输入和助手回复的格式
    # 对应你想要的: <|user|>\nHow are you doing?\n<|assistant|>\n
    prompt: Prompt = field(default_factory=lambda: [
        '<|user|>\n{{QUERY}}\n<|assistant|>\n'
    ])
    
    # 3. 对话分隔符：多轮对话中，助手回复完后的结束符
    ...
    # 5. 系统提示词格式 (可选)
    # 为了保持风格一致，建议将 System 也改为类似格式，例如 <|system|>\n...
    # 如果不需要 Llama 3.2 特有的日期注入，可以直接写死：
    system_prefix: Optional[Prompt] = field(
