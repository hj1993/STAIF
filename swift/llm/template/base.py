...
class Template(ProcessorMixin):
    ...
    def _rlhf_encode(self, inputs: TemplateInputs) -> Dict[str, Any]:
        ...
        num_rejected = 0
        if inputs.rejected is not None:
            rejected_encoded = self._encode_truncated(inputs.rejected)
            num_rejected = 1
        elif chosen.rejected_response:
            rejected_encoded = {}
            for rejected_response in chosen.rejected_response:
                negative = deepcopy(chosen)
                negative.messages[-1]['content'] = rejected_response
                negative.rejected_response = []
                negative_encoded = self._encode_truncated(negative)
                for k, v in negative_encoded.items():
                    rejected_encoded.setdefault(k, []).append(v)
            num_rejected = len(chosen.rejected_response)
        else:
            raise ValueError('RLHF training requires either `rejected` or `chosen.rejected_response`.')
        ...
        encoded['num_rejected'] = num_rejected
    ...
    def _rlhf_data_collator(self,
        ...
        chosen_batch = self._fetch_inputs_startswith(batch, chosen_prefix)
        rejected_batch = []
        num_rejected = []
        for inputs in batch:
            rejected_inputs = {k[len(rejected_prefix):]: v for k, v in inputs.items() if k.startswith(rejected_prefix)}
            sample_num_rejected = int(inputs.get('num_rejected', 1))
            num_rejected.append(sample_num_rejected)
            if sample_num_rejected == 1:
                rejected_batch.append(rejected_inputs)
                continue
            for i in range(sample_num_rejected):
                rejected_batch.append({k: v[i] for k, v in rejected_inputs.items()})
        res = self._data_collator(chosen_batch + rejected_batch, padding_to=padding_to)
        res['num_rejected'] = torch.tensor(num_rejected, dtype=torch.long)
        res['num_chosen'] = len(chosen_batch)
