...
class MegatronDPOTrainer(MegatronTrainer):
    ...
    @staticmethod
    def _split_rejected_by_group(rejected_tensor: torch.Tensor, num_rejected: torch.Tensor):
        groups = []
        start = 0
    ...
    def multi_negative_dpo_loss(self, logps, ref_logps, num_samples, num_rejected):
        chosen_logps = logps[:num_samples]
        rejected_logps = logps[num_samples:]
        ref_chosen_logps = ref_logps[:num_samples]
        ref_rejected_logps = ref_logps[num_samples:]
        losses, chosen_rewards, rejected_rewards = self.dummy_dpo_trainer.multi_negative_dpo_loss(
            chosen_logps,
            rejected_logps,
            ref_chosen_logps,
            ref_rejected_logps,
    ...
    def loss_func(self, output_tensor: torch.Tensor, *, ref_logps: torch.Tensor, labels: torch.Tensor,
                  packed_seq_params, num_rejected: torch.Tensor = None):
        args = get_args()
        num_samples = packed_seq_params.num_samples

        logps = self.get_logps(output_tensor, labels, packed_seq_params)
        if num_rejected is not None and torch.any(num_rejected != 1):
            loss, chosen_rewards, rejected_rewards = self.multi_negative_dpo_loss(
                logps, ref_logps, num_samples, num_rejected)
        else:
    ...
    def forward_step(self, data_iterator, model):
        ...
        return output_tensor, partial(
            ...
            num_rejected=data.get('num_rejected'))
